...
import type { ReviewConfig, PullRequestInfo } from "../types/index.js";
...
export class GitHubService {
  ...
  async getPullRequestInfo(): Promise<PullRequestInfo> {
    ...
    if (!pullRequest) {
      throw new Error(
        "이 액션은 Pull Request 이벤트에서만 실행될 수 있습니다."
      );
    }
  ...
  async getDiff(): Promise<string> {
    ...
    try {
      ...
      if (
        this.config.maxPatchLength &&
        diff.length > this.config.maxPatchLength
      ) {
        throw new Error(
          `변경사항이 너무 큽니다: ${diff.length} > ${this.config.maxPatchLength}`
        );
      }
  ...
  async createReviewComment(review: string): Promise<void> {
    const prInfo = await this.getPullRequestInfo();
